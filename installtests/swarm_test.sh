...
echo "Extracting the UCN installer"
...
ssh $ssh_opts -i $ssh_key $user@$ucn_master "cd $dest_path/$install_version && chmod +x install/ansible/install_swarm.sh"
echo "Loading installer and proxy docker images" 
ssh $ssh_opts -i $ssh_key $user@$ucn_master "cd $dest_path/$install_version && sudo ./install/install.sh"
# Do a docker run here mounting the private key and other ansible opts
echo "Starting the ansible container"
ssh $ssh_opts -i $ssh_key $user@$ucn_master "cd $dest_path/$install_version && ./install/ansible/install_swarm.sh \"-n $ucn_master -f install/ansible/cfg.yml -i\""
